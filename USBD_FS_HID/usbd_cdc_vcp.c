...
static uint16_t VCP_DataTx_Len   (uint8_t* Buf, uint32_t Len);
static uint16_t VCP_DataRx_Len   (uint8_t* Buf, uint32_t Len);
...
//--------------------------------------------------------------
uint16_t VCP_StringRx(char *ptr)
{
  uint16_t akt_pos=0;
  uint8_t wert;
  uint32_t temptail;
