...
//#define HFP  2
//#define VFP   2
#define HBP  134//46		//HSYNC后的无效像素
#define VBP  11// 50//23		//VSYNC后的无效行数

#define HSW   10//1		//HSYNC宽度
#define VSW   2//1		//VSYNC宽度

#define HFP  16//16//22		//HSYNC前的无效像素
#define VFP   32//22		//VSYNC前的无效行数
...
unsigned int findHzIndex_24(const unsigned char *hz)                                                                       
{
    uint16_t j;
  //NT_GB16 *ptGb16 = (FNT_GB16 *)GBHZ_16;
	...
	{
        //if ((*hz == ptGb16->Index[0]) && (*(hz+1) == ptGb16->Index[1])) FNT_GB16//&&(*(hz+1) == GB_21[j].Index[1])
		if((GB_24[j].Index[0]==(*hz))&&(*(hz+1)==GB_24[j].Index[1]))
        {
            return j;
        }
    }
...
unsigned int findHzIndex_32(const unsigned char *hz)                                                                       
{
    uint16_t j;
    for(j=0;j<sizeof(GB_32)/sizeof(GB_32[0]);j++)
		{ 
				if(( GB_32[j].Index[0] == (*hz) )&&( *(hz+1) == GB_32[j].Index[1] ))
        {
            return j;
        }
    }
...
unsigned int search_MSFont_Index( const unsigned char *hz ){

	 uint16_t j;
	 
	 for( j = 0 ; j < sizeof(MS24x23)/sizeof(MS24x23[0]); j++){
				 
			 if( (MS24x23[j].Index[0] == (*hz)) && (*(hz+1) == MS24x23[j].Index[1]) ){
...
{
	LCD429_PutPixel(x, y, POINT_COLOR );
}
...
void LCD_DrawChar(uint16_t Xpos, uint16_t Ypos, const uint16_t *c)
{
	...
	u32 *pt1;
	u8 *pt;
	Typedf_Disp_Colour color;
	color.black=Colour.black;
	color.Fword=color.Fword;
	
	for(t=0;t<(LCD_Currentfonts->Height)*(LCD_Currentfonts->Width/8);t++)
	{   
			for(t1=0;t1<8;t1++)
			{
				if(c[t]<<t1 & 0x80)
					LCD429_PutPixel(Xpos,Ypos,Colour.Fword);
				else 
					LCD429_PutPixel(Xpos,Ypos,Colour.black);
				   
				Xpos++;
				if(Xpos-x==LCD_Currentfonts->Width)
				{
					Xpos=x;
					Ypos++;                        
					break;
...
void LCD_DrawChar32(uint16_t x, uint16_t y, const uint8_t *c)
{
...
void LCD_DisplayChar32(uint16_t Line, uint16_t Column, uint8_t Ascii)
{
   Ascii -= 32;
...
void LCD_ShowFontCN_24(uint16_t x,uint16_t y,uint16_t fno)
{                                  
	uint8_t temp,t,t1;
	uint16_t x0=x;
	temp=0;  
	...
	{        
		for(t=0;t<24*3;t++)
		{   
			temp=GB_24[fno].Msk[t];                //gb16Dot[fno][t];    //调用16*08??   
...
void LCD_DisplayFont_MS24x23( uint16_t x, uint16_t y, uint16_t fno ){
...
void LCD_ShowFontCN_32(uint16_t x,uint16_t y,uint16_t fno)
{                                  
	uint8_t temp,t,t1;
	uint16_t x0=x;
	temp=0;  
	...
	{        
		for(t=0;t<32*4;t++)
		{   
			temp=GB_32[fno].Msk[t];                //gb16Dot[fno][t];    //调用16*08??   //GB_32
...
void Disp_Usbflag(uint8_t flag)
{
	...
	if(flag==1)
		...
		Colour.Fword=LCD_COLOR_RED;
	LCD_ShowFontCN_40_55(600-160,4,32,32,(uint8_t*)nBitmapDot);
  Colour.Fword=colour;
}
...
//	uint16_t usIndex=0;
	...
	while(1)
	{
		...
		if(*pcStr>='0'&&*pcStr<='9')
		{
			...
			i=12;
		if(fonts==1)//Channel_97x48_Table
        {
            wide=16;
            high=24;
            pt=(uint8_t*)Out_1224;
		...
		else if(fonts==2)
		{
			wide=48;
            high=97;
            pt=(uint8_t*)Channel_97x48_Table;
...
void Disp_R_V(void)
{
		LCD_ShowFontCN_40_55( TESTVALUE_X+48*9, 200-2, 40, 55,(uint8_t*)Out_Assic+20*40*55/8 );     //Ω

		LCD_ShowFontCN_40_55( TESTVALUE_X+48*9, 300-2, 40, 55,(uint8_t*)Out_Assic+27*40*55/8 );  //V
...
// }
	...
	{	
		...
		usCh = search_MSFont_Index( t_char );
		...
		// 0xA6 && 0xB8   This is "Ω"    MS24x23[69]
		// 0x3A           This is "："    MS24x23[95]	
		// 0x2E           This is "."     MS24x23[94] 
		// 0x20           This is " "     MS24x23[231]
		// 0X3C/0X3E      This is "<>"    MS24x23[170] / MS24x23[171]
...
void LCD_Draw_Circle(u16 x,u16 y,u16 r)
{
	LCD_DrawCircle(LCD_PIXEL_WIDTH-x,LCD_PIXEL_HEIGHT-y,r);
...
GPIO_ResetBits(GPIOD,GPIO_Pin_13);//y
